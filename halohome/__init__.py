...
class Group:
    ...
    async def set_brightness(self, brightness: int) -> bool:
        return await self.location.set_brightness(self.group_id, brightness)

    async def set_color_temp(self, color: int) -> bool:
        return await self.location.set_color_temp(self.group_id, color)
...
class LocationConnection:
    ...
    def __init__(self, location_id: str, passphrase: str, devices: List[dict], groups: List[dict],timeout: int = TIMEOUT):
        self.devices = []
        self.groups = []
...
async def _load_groups(
    ...
    response = await _make_request(
        host, f"locations/{location_id}/groups", auth_token=auth_token, timeout=timeout
    )
...
async def _load_location(host: str, auth_token: str, location_id: int, timeout: int) -> dict:
    ...
    devices, groups = await asyncio.gather(
        _load_devices(host, auth_token, location_id, timeout),
        _load_groups(host, auth_token, location_id, timeout)
    )
    return {"location_id": raw_location["pid"], "passphrase": raw_location["passphrase"], "devices": devices, "groups": groups}
